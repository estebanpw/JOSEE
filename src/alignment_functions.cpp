    ...
    */
	...
	for(i=1;i<(int64_t) Xend;i++){
        ...
        */
		...
		for(j=MAX(1,(cell_path_y[i] - window_size));j<MIN((int64_t)Yend,(cell_path_y[i] + window_size));j++){
            ...
            */
			...
			if(i == (int64_t) Xend-1 || j == (int64_t) Yend-1){

				if(i == (int64_t) Xend-1 && j != (int64_t) Yend-1){
            		table[i][j_prime].score = table[i][j_prime].score + iGap + (Yend - j)*eGap;
				}else if(j == (int64_t) Yend-1 && i != (int64_t) Xend-1){
